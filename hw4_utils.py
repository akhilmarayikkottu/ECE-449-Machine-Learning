...
def contour_plot_p(xmin, xmax, ymin, ymax, pred_fxn, ngrid = 33):
    """
    make a contour plot
    ...
    plt.plot( 1, 1, 'rX',markersize=8)
    plt.plot(-1,-1, 'rX',markersize=8)
    plt.plot(-1, 1, 'bX',markersize=8)
    plt.plot( 1,-1, 'bX',markersize=8)
    ...
    C = plt.contour(xx, yy, zz,
                    cmap = 'coolwarm')
    plt.clabel(C)
    plt.savefig('Xor_p.pdf')
    plt.show()

    return plt.gcf()


def contour_plot_l(xmin, xmax, ymin, ymax, pred_fxn, ngrid = 33):
    # Build grid
    xgrid = torch.linspace(xmin, xmax, ngrid)
    ygrid = torch.linspace(ymin, ymax, ngrid)
    (xx, yy) = torch.meshgrid(xgrid, ygrid)
